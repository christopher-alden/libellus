...
export default function ContentCard({ courses, category }) {
  ...
  return (
    <Link to={`/details/${category}/${courses.id}`}>
      <div className={`text-d-text bg-d-secondary w-80 h-64 rounded-md ${brColor} border-2 flex-col shadow-md mx-auto`}>
        <div
          className={`w-full ${bgColor} bg-d-easy px-4 py-2 rounded-t-sm  text-md font-medium`}
        >
          {courses.difficulty}
        </div>
        <div className="p-4">
